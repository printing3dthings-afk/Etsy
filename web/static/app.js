...
function selectAgent(name) {
  ...
  document.getElementById("header-icon").style.background = hexToRgba(color, 0.15);
  renderMessages();
}
...
function renderMessages() {
  ...
  box.innerHTML = "";
  history.forEach(m => box.appendChild(buildMessageEl(m)));
    ...
    es.onmessage = e => {
      try {
        ...
        if (data.type === "status") {
          ...
          es.close(); setThinking(false);
          addMessage("agent", data.text, statuses); resolve();
        } else if (data.type === "error") {
          es.close(); setThinking(false);
          addMessage("agent", `Error: ${data.text}`); reject(new Error(data.text));
        }
      } catch (_) {}
    };
    es.onerror = () => { es.close(); setThinking(false); reject(new Error("Connection lost")); };
  });
}
