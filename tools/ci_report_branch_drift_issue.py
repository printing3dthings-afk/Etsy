...
result of tools/check_default_branch.py (.github/workflows/health_watchdog.yml).

Usage:
    python tools/ci_report_branch_drift_issue.py <ok|fail> [details-text]

Reuses the same one-persistent-issue-by-marker-title pattern as
tools/ci_report_health_issue.py so Scott gets one running thread per incident
instead of issue spam. On recovery (status=ok with an existing open tracking
issue), comments "recovered" and closes the issue rather than leaving it open
forever. Requires GH_TOKEN in the environment (the default GITHUB_ACTIONS
...
MARKER_TITLE = "GitHub default branch has drifted"
LABEL = "branch-drift"
...
def find_open_issue(repo: str, token: str) -> dict | None:
    # urlencode (not raw f-string interpolation) -- see ci_report_health_issue.py's
    # docstring for the exact InvalidURL crash this avoids (em dash + quotes in the
    # query trip Python's http.client control-character validator).
    ...
    try:
        ...
        try:
            gh_request(
                ...
                token,
                body={"name": LABEL, "color": "b60205",
                      "description": "GitHub default_branch drifted from the active working branch"},
            )
        except Exception:
...
def main() -> int:
    if len(sys.argv) < 2 or sys.argv[1] not in ("ok", "fail"):
        print("Usage: ci_report_branch_drift_issue.py <ok|fail> [details-text]", file=sys.stderr)
        return 1
    ...
    if status == "fail":
        body = (
            f"GitHub's default_branch setting has drifted as of {now}.\n\n"
            f"Every schedule-triggered workflow silently resolves against whatever this "
            f"points to — this is the same class of incident as 2026-07-10, where a stale "
            f"default branch caused cron jobs to run old, broken code for an unknown period.\n\n"
    ...
    if existing:
        gh_request(
            ...
            f"https://api.github.com/repos/{repo}/issues/{existing['number']}/comments",
            token,
            body={"body": f"Recovered as of {now}. default_branch matches the expected "
                          f"active branch again.\n\nWorkflow run: {run_url}"},
        )
        gh_request(
            "PATCH",
        ...
        print("Branch check OK, no open issue to close.")
    return 0
