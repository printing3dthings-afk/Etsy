...
# A pixel is background if it is within this Euclidean RGB distance of the
# sampled sheet-background color. Sticker sheets are generated on off-white /
# cream paper (~RGB 240,237,232) — NOT pure white — so a hard "all channels >=238"
# test misses the background entirely and the whole sheet fuses into one blob.
# That is the DP1027 Sheet 6 "stickers too connected" defect: the stickers were
...
def remove_white_background(img: Image.Image) -> Image.Image:
    ...
    # Sample the background from the four corners (median = robust to a sticker that
    # happens to reach into one corner).
    c = 24
    ...
    if n == 0:
        # No background-colored pixels at all — fully opaque
        alpha = np.full((h, w), 255, dtype=np.uint8)
        return Image.fromarray(np.dstack([rgb.astype(np.uint8), alpha]))

    # Any background-colored region touching the border is the sheet background.
    border_labels = set()
    border_labels.update(np.unique(labeled[0, :]).tolist())
    border_labels.update(np.unique(labeled[-1, :]).tolist())
    border_labels.update(np.unique(labeled[:, 0]).tolist())
    border_labels.update(np.unique(labeled[:, -1]).tolist())
    border_labels.discard(0)  # 0 = non-background, never removed by this rule

    background = np.isin(labeled, list(border_labels))

    # Soften the cut edge by 1px to reduce background haloing (erode the opaque area
    # slightly so anti-aliased fringe pixels are dropped).
    opaque = ~background
