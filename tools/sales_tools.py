...
TOOL_DEFINITIONS = [
    {"name": "get_orders", "description": "Retrieve orders filtered by status.",
     "input_schema": {"type": "object", "properties": {"status": {"type": "string", "enum": ["all", "payment_complete", "shipped", "complete"]}}, "required": ["status"]}},
    {"name": "get_order_details", "description": "Get full details of a specific order.",
     "input_schema": {"type": "object", "properties": {"order_id": {"type": "string"}}, "required": ["order_id"]}},
    {"name": "get_revenue_summary", "description": "Get revenue statistics for a time period.",
    ...
    {"name": "update_order_status", "description": "Update the status of an order.",
     "input_schema": {"type": "object", "properties": {"order_id": {"type": "string"}, "new_status": {"type": "string", "enum": ["payment_complete", "shipped", "complete", "cancelled"]}, "tracking_number": {"type": "string"}}, "required": ["order_id", "new_status"]}},
    {"name": "get_shipping_queue", "description": "Get all paid orders waiting to ship.",
     "input_schema": {"type": "object", "properties": {}, "required": []}},
]


def execute_tool(tool_name: str, tool_input: dict, store: DataStore) -> str:
    if tool_name == "get_orders":         return _get_orders(tool_input["status"], store)
    if tool_name == "get_order_details":  return _get_order_details(tool_input["order_id"], store)
    if tool_name == "get_revenue_summary":return _get_revenue_summary(tool_input["period"], store)
    if tool_name == "update_order_status":return _update_order_status(tool_input, store)
    if tool_name == "get_shipping_queue": return _get_shipping_queue(store)
...
def _get_revenue_summary(period: str, store: DataStore) -> str:
    ...
    return json.dumps({
        "period": period, "revenue": amount, "currency": "USD",
...
def _update_order_status(tool_input: dict, store: DataStore) -> str:
    ...
    store.save()
    result = {"success": True, "order_id": order_id, "previous_status": old_status,
              "new_status": new_status, "updated_at": str(date.today())}
    if tracking:
